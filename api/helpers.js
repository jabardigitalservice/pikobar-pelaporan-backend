...
function CommentReferenceError(message) {
  ...
  this.message = message || 'This comment does not belong to the pelaporan apps !'
...
const checkRole = (server) => {
  const roleView = require('./users/route_prerequesites').CheckRoleView(server)
  const roleRud = require('./users/route_prerequesites').CheckRoleCreate(server)

  return {
    viewOnly: roleView,
    createUpdateDelete: roleRud
  }
}
